...
class VWtxtCreator(object):
    ...
    def create_df_vw(self, _df, is_train):
        ...
        # it's slow..
        df['datestr'] = list(
            map(lambda d: bytes(d.strftime('%Y%m%d'), 'utf8'), df.date2))
        df['id2'] = (np.char.array(df.item_nbr) + bytes("_", 'utf-8') +
                     np.char.array(df.store_nbr) + bytes("_", 'utf-8') + df.datestr)
        ...
        # use for training, but predict as zero
        df['include_prediction'] = df.include3
    ...
    def write_txt(self, _df, fname):
        ...
        for i, row in _df.iterrows():
            ...
            newline += " |M day{} month{} year{}".format(
                row.day, row.month, row.year)
            newline += " |W isRS:{} departF:{}".format(
                row.preciptotal_flag, row.depart_flag)
            newline += " |I id {} avl4 {} rmean {}".format(
                row.id2.decode('utf-8'), row.include_prediction, row.baseline)
